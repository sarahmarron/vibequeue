# Project VibeQueue Description

Our app is a full-stack web application that integrates a React frontend with a Django backend to support natural-language–driven music discovery and Spotify playback. Users authenticate via Spotify OAuth, submit free-form text prompts describing a desired musical vibe, and receive structured song suggestions generated by the OpenAI API, which are seamlessly played on the user's device. The backend processes prompts, stores returned song metadata in a database, resolves suggested tracks to Spotify track IDs via search endpoints, and queues playable tracks to the user’s active Spotify device. The frontend displays prompt results in real time and allows users to initiate playback directly through the application.

-----

# Setup Guide (Requires OpenAI API key, Spotify Developer App permissions/setup, and Spotify premium)
